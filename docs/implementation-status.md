...
Updated September 19, 2026. This document describes the current source and validation evidence. The [product brief](product-brief.md) remains the requested scope; the [implementation plan](implementation-plan.md) contains earlier proposed decisions.
...
S1-mini cold-loads per helper request and accepts at most 1,000 transcript tokens. The native caller splits longer text at sentence/word boundaries and retries smaller chunks if the helper rejects an oversized chunk. Each helper job has a 120-second timeout, and startup removes stale job files. Meaning across chunk boundaries and total cleanup latency need testing. S1-mini's formatting controls do not provide general-purpose custom prompting or meeting summaries.
