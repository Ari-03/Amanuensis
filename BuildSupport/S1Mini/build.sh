...
"$s1_cmake" -S "$s1_root" -B "$s1_root/.build" \
  -DLLAMA_SOURCE="$s1_source" -DCMAKE_BUILD_TYPE=Release \
  -DCMAKE_OSX_SYSROOT="$(xcrun --sdk macosx --show-sdk-path)" \
  -DCMAKE_OSX_DEPLOYMENT_TARGET=14.0 \
  -DCMAKE_OSX_ARCHITECTURES="${S1MINI_ARCH:-arm64}"
"$s1_cmake" --build "$s1_root/.build" --target S1MiniHelper --parallel "$(sysctl -n hw.ncpu)"
cp "$s1_root/.build/S1MiniHelper" "$s1_root/dist/S1MiniHelper"
mkdir -p "$s1_root/dist/licenses"
cp "$s1_root"/licenses/* "$s1_root/dist/licenses/"
